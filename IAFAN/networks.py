...
resnet_dict = {"resnet50":models.resnet50, "resnet101":models.resnet101, 'alexnet':models.alexnet, 'vgg16':models.vgg16}

class ResNetFc(BaseFeatureExtractor):
    def __init__(self, device, model_name='resnet50',model_path=None, normalize=True):
        super(ResNetFc, self).__init__()
        self.model_resnet = resnet_dict[model_name](pretrained=False)
        pre = torch.load(model_path)  # 进行加载
        self.model_resnet.load_state_dict(pre)
...
class AlexNetFc(BaseFeatureExtractor):#
    def __init__(self, device, model_name='alexnet', model_path=None, normalize=True):
        ...
        if model_path or normalize:
            ...
            self.normalize = False
        self.features_ = model_alexnet.features
        self.avgpool_ = model_alexnet.avgpool #6,6,256
        self.feature_layers = nn.Sequential(self.features_, self.avgpool_)
        self.device = device
...
class CLS(nn.Module):
    def __init__(self, in_dim, out_dim, bottle_neck_dim=256):
        ...
        if bottle_neck_dim:
            self.bottleneck = nn.Linear(in_dim, bottle_neck_dim)    #2048->256
            self.fc = nn.Linear(bottle_neck_dim, out_dim)           #256->26
